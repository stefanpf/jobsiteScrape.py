...
def scrape_bsj(category, old_time):
    ...
    while not list_exhausted:
        print("Scraping berlinstartupjobs.com/%s/page/%s/ for links..."
              % (category, str(page_number)))
        res = requests.get('http://berlinstartupjobs.com/%s/page/%s/'
                           % (category, str(page_number)),
        ...
        print("Finished scraping berlinstartupjobs.com/%s/page/%s/"
              % (category, str(page_number)))
        page_number += 1
